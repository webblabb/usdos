...
USAMM_parameters::USAMM_parameters(const Parameters* p, Farm_type* ft) :
    p(p), farm_type(ft)
{
...
void USAMM_parameters::initialize_parameters(std::string fname)
{
    ...
    if(f.is_open())
    {
        ...
        for(size_t col_i = 0; col_i < header_vector.size(); col_i++)
        {
            ...
            if(colname_vector.size() == 3 and colname_vector.at(2).compare("like") != 0)
            {
                ...
                {
                    //Origin covariate
                    ocov_parameter_map[temporal_name][colname_vector.at(2)] = this_val;
                    USAMM_time_periods.insert(temporal_name);
                }
                else if(colname_vector.at(0).compare("i") == 0)
                {
                    //Destination covariate
                    dcov_parameter_map[temporal_name][colname_vector.at(2)] = this_val;
                    USAMM_time_periods.insert(temporal_name);
                }
                else
                {
...
void USAMM_parameters::initialize_covariates(std::string fname, str_vec_map& cov_map,
...
void USAMM_parameters::initialize_supernode_cov(std::string fname, str_vec_map& ocov_map,
                                                str_vec_map& dcov_map)
{
    if(fname.compare("*") != 0)
    {
        ...
        if(f.is_open())
        {
            ...
            std::string line;
            std::vector<std::string> line_vector;
            ocov_parameter_names.push_back(std::string("SuperShipper"));
            dcov_parameter_names.push_back(std::string("SuperReceiver"));
            while(std::getline(f, line))
            {
                line_vector = split(line, '\t');
                if(!line_vector.empty())
...
Farm_type* USAMM_parameters::get_farm_type()
{
    return farm_type;
}

double USAMM_parameters::get_std(int state_code, std::string time_period)
{
    return std_map.at(state_code).at(time_period);
}

double USAMM_parameters::get_kurt(int state_code, std::string time_period)
{
    return kurt_map.at(state_code).at(time_period);
}

double USAMM_parameters::get_a(int state_code, std::string time_period)
{
...
double USAMM_parameters::get_b(int state_code, std::string time_period)
{
    //If the value have been calculated, return that.
...
double USAMM_parameters::get_N(int state_code, std::string time_period)
{
    double temp_N;
    try
    {
...
double USAMM_parameters::get_lambda(int state_code, std::string time_period)
{
    double temp_lambda;
    try
    {
...
double USAMM_parameters::get_s(int state_code, std::string time_period)
{
    return s_map.at(state_code).at(time_period);
}

std::vector<double> USAMM_parameters::get_county_o_covs(County* c)
{
    if(o_covariates_loaded)
    {
        std::string county_id = c->get_id();
        bool has_farms = c->get_n_farms(this->farm_type) > 0;
        ...
        else if(has_farms and !has_covariates)
        {
            std::cout << "The county " << county_id << " has farms of type "
                      ...
                      << std::endl;
            return std::vector<double>(ocov_parameter_names.size(), 0.0);
        }
    }
    return std::vector<double>(ocov_parameter_names.size(), 0.0);
}

std::vector<double> USAMM_parameters::get_county_d_covs(County* c)
{
    if(d_covariates_loaded)
    {
        ...
        else if(has_farms and !has_covariates)
        {
            std::cout << "The county " << county_id << " has farms of type "
                      ...
                      "associated with that type. Setting those covariates to 0.0"
                      << std::endl;
            return std::vector<double>(dcov_parameter_names.size(), 0.0);
        }
    }
    return std::vector<double>(dcov_parameter_names.size(), 0.0);
}

std::vector<std::string> USAMM_parameters::get_ocov_names()
{
    return ocov_parameter_names;
}

std::vector<std::string> USAMM_parameters::get_dcov_names()
{
    return dcov_parameter_names;
}

double USAMM_parameters::get_ocov_parameter(std::string par_name, std::string time_period)
{
    double temp_cov;
    try
    {
        temp_cov = ocov_parameter_map.at(time_period).at(par_name);
    }
    catch(const std::out_of_range& e)
    {
...
double USAMM_parameters::get_dcov_parameter(std::string par_name, std::string time_period)
{
    double temp_cov;
    try
    {
        temp_cov = dcov_parameter_map.at(time_period).at(par_name);
    }
    catch(const std::out_of_range& e)
    {
        std::cout << "The covariate with the name " << par_name << " (" << time_period << ") "
...
std::string USAMM_parameters::get_generation_string()
{
    return generation_string;
}

std::vector<double> USAMM_parameters::calculate_a_and_b(double dx1, double R,
...
bool USAMM_parameters::get_parameters(double stdev, double kurt, double const& epsilon,
...
double USAMM_parameters::log_int_hal(double n0,double n1, double epsilon, double log_kurtosis,
                                     bool& status)
{
    double check;
    status = true;
...
double USAMM_parameters::log_kurtosisf(double n, double log_kurtosis, bool& status)
{
    double res = 0.0;
    try
    {
...
double USAMM_parameters::return_log_a(double n, double sd)
{
    return std::log(sd) + (std::lgamma(2/n) - std::lgamma(4/n))/2;
}

bool USAMM_parameters::get_one_random_line(std::ifstream& f, std::string& res_line, bool header)
{
    res_line.clear();
    f.seekg(0, f.beg);
    ...
    if(f.is_open())
    {
        ...
        while(res_line.empty())
         {
             get_this_line = rand_int(start_at, n_lines);
...
bool USAMM_parameters::temporal_name_exists(std::string temporal_name)
{
    bool exists = false;
